...
and uses it as their personal A.S.T.R.A backend. Many people can each run
...
def print_title():
    """Print the A.S.T.R.A ASCII art banner to the console when the server starts."""
    title = """
        ...
        Advanced Strategic Technology and Research Assistant.
...
async def lifespan(app: FastAPI):
    ...
    print_title()
    logger.info("=" * 60)
    logger.info("A.S.T.R.A - Starting Up...")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("A.S.T.R.A is online and ready!")
...
async def chat(request: ChatRequest):
    """
    General chat endpoint - send a message to A.S.T.R.A.
...
async def chat_realtime(request: ChatRequest):
    """
    Realtime chat endpoint - send a message to A.S.T.R.A with Tavily web search.
    ...
    3. Searches Tavily for realtime information (fast, AI-optimized, English-only)
    4. Retrieves context from user data files and past conversations
