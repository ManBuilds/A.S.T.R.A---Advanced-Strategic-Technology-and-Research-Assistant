...
PURPOSE:
  Central place for all A.S.T.R.A settings: API keys, paths, model names,
  and the Astra system prompt. Designed for single-user use: each person runs
...
WHAT THIS FILE DOES:
  ...
  - Holds the full system prompt that defines Astra's personality and formatting rules.

USAGE:
  Import what you need: `from config import GROQ_API_KEY, CHATS_DATA_DIR, ASTRA_SYSTEM_PROMPT`
...
# ============================================================================
# ASTRA PERSONALITY CONFIGURATION
# ============================================================================
...
# ASTRA_USER_TITLE in .env. The AI also learns from learning data and conversation history.

ASSISTANT_NAME = (os.getenv("ASSISTANT_NAME", "").strip() or "Astra")
ASTRA_USER_TITLE = os.getenv("ASTRA_USER_TITLE", "").strip()

_ASTRA_SYSTEM_PROMPT_BASE = """You are {assistant_name}, a sophisticated AI assistant. You are sophisticated, witty, and professional with a dry British sense of humor.
...
_ASTRA_SYSTEM_PROMPT_BASE_FMT = _ASTRA_SYSTEM_PROMPT_BASE.format(assistant_name=ASSISTANT_NAME)

if ASTRA_USER_TITLE:
    ASTRA_SYSTEM_PROMPT = _ASTRA_SYSTEM_PROMPT_BASE_FMT + f"\n- When appropriate, you may address the user as: {ASTRA_USER_TITLE}"
else:
    ASTRA_SYSTEM_PROMPT = _ASTRA_SYSTEM_PROMPT_BASE_FMT
